...
RenderableInteraction::RenderableInteraction(const Interaction& interaction, Renderer* renderer) :
  interaction_(interaction), renderer_(renderer) {}

auto RenderableInteraction::draw(const int y_offset) const -> void {
  if (this->interaction_.is_self_referential()) { this->draw_self_referential_interaction(y_offset); }
  else { this->draw_point_to_point_interaction(y_offset); }
}
...
auto RenderableInteraction::draw_point_to_point_interaction(const int y_offset) const -> void {
  ...
  const auto line_y = y_offset + (this->interaction_.get_index() * LayoutConstants::INTERACTION_GAP) +
    LayoutConstants::INTERACTION_GAP;
...
auto RenderableInteraction::draw_self_referential_interaction(const int y_offset) const -> void {
  ...
  const int from_line_y = y_offset + (this->interaction_.get_index() * LayoutConstants::INTERACTION_GAP) +
    LayoutConstants::INTERACTION_GAP;
...
auto RenderableInteraction::draw_arrowhead(const int line_end_x, const int line_end_y) const -> void {
  ...
  if (is_pointing_right) {
    if (this->interaction_.is_async()) {
      ...
      renderer_->draw_arrowhead(QPoint(line_end_x, line_end_y), RenderingUtils::ArrowDirection::Right,
                                RenderingUtils::ArrowStyle::Open);
    }
    else {
      // draw filled for sync
      renderer_->draw_arrowhead(QPoint(line_end_x, line_end_y), RenderingUtils::ArrowDirection::Right,
                                RenderingUtils::ArrowStyle::Closed);
    }
  }
  else {
    if (this->interaction_.is_async()) {
      // draw /
      renderer_->draw_arrowhead(QPoint(line_end_x, line_end_y), RenderingUtils::ArrowDirection::Left,
                                RenderingUtils::ArrowStyle::Open);
    }
    else {
...
auto RenderableInteraction::render_interaction_message(const int interaction_from_x, const int interaction_from_y,
  ...
  if (!this->interaction_.get_message().empty()) {
    ...
    const int message_width = RenderingUtils::get_font_rendered_width(this->interaction_.get_message(),
                                                                      renderer_->get_message_font());
    ...
    int height = LayoutConstants::MESSAGE_X_PADDING + RenderingUtils::get_font_rendered_height(
      renderer_->get_message_font());
    renderer_->draw_text(label_x, interaction_from_y - height, this->interaction_.get_message().c_str(),
                         renderer_->get_message_font());
  }
}
...
auto RenderableInteraction::get_rightmost_x() const -> int {
  const auto line_from_x = get_participant_x(this->interaction_.get_from());
  if (is_pointing_right()) {
    const auto font_rendered_width = RenderingUtils::get_font_rendered_width(
      this->interaction_.get_message(), renderer_->get_message_font());
    return (2 * LayoutConstants::DIAGRAM_MARGIN) + (line_from_x + font_rendered_width) +
      LayoutConstants::MESSAGE_X_PADDING;
  }
  return line_from_x;
}
