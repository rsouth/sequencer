...
public:
  ...
  static std::string join(const std::vector<std::string>& lines, const std::string join_with) {
    return std::accumulate(
      ...
      lines.end(),
      lines.front(),
      [join_with](std::string a, std::string b) { return a + join_with + b; }
    );
  }
  ...
  static void replace_token_and_value(std::vector<std::string>& lines, std::string token, std::string value) {
    ...
    if (!replaced) { lines.insert(lines.begin(), 1, full_string); }
  }
};
