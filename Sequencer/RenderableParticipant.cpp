...
RenderableParticipant::RenderableParticipant(const Participant& participant, Renderer* renderer) :
  participant_(participant), renderer_(renderer) {}
...
void RenderableParticipant::draw(const int header_y_offset, const int total_interactions) {
  ...
  // render rectangle...
  this->renderer_->draw_rectangle(QPoint(participant_x0, participant_y0),
                                  QPoint(LayoutConstants::LANE_WIDTH, LayoutConstants::LANE_HEIGHT));
  const std::string partic_name = this->participant_.get_name();

  const int text_x = get_participant_x() + (LayoutConstants::LANE_WIDTH / 2) - (RenderingUtils::get_font_rendered_width(
    partic_name, renderer_->get_header_font()) / 2);

  const int text_y = header_y_offset + LayoutConstants::V_GAP + (LayoutConstants::LANE_HEIGHT / 2) - (
    RenderingUtils::get_font_rendered_height(renderer_->get_header_font()) / 2);
  ...
  const int y2 = y1 + LayoutConstants::INTERACTION_GAP + (total_interactions * (LayoutConstants::INTERACTION_GAP +
    LayoutConstants::V_GAP));

  // last VGAP should be a 'lane height padding' type.
  renderer_->draw_line(QPoint(participant_x0 + LayoutConstants::LANE_WIDTH / 2, y1),
                       QPoint(participant_x0 + LayoutConstants::LANE_WIDTH / 2, y2));
}
